...
# ---------------------------------------------------------------------------
# Train-time temporal context (yamnet-aves-context)
# ---------------------------------------------------------------------------
#
...
# Measured hit rate on medium/yamnet_aves: 92.1% each side, 84.7% both.
#
...
# yamnet_aves is [YAMNet 1024 | AVES 768], and stacking just the YAMNet block
# reproduces `yamnet_context`'s 3072-d input beside an unwidened AVES block
# (3840-d total) rather than also inventing an untested AVES-context lever.

CONTEXT_FRAMES = 0
CONTEXT_DIMS = 0        # 0 = the whole embedding


def set_context(frames, dims=0):
    global CONTEXT_FRAMES, CONTEXT_DIMS
...
def apply_context(samples):
    ...
    if n_tot:
        print(f'  context k={k}: {n_hit}/{n_tot} neighbours real '
