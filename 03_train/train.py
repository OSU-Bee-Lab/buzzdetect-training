...
class TrainingData:
    ...
    # standardization layer (--standardize). Fit inside the fold, never on the
    # held-out one -- see _input_stats. None when --standardize is off.
...
# unchanged (mean 0, variance 1) instead of being scaled. Keras'
# Normalization divides by sqrt(var + 1e-7), so a dim that never varies would
# be amplified by ~1e4 into pure noise. Floor chosen an order of magnitude
# above that epsilon.
VAR_FLOOR = 1e-6


def _input_stats(samples):
    """Per-dim (mean, variance) of a training pool, plus a pass-through count.

    Fold safety: the caller passes the *training* samples only, so the
    standardization applied at validation and scoring time is a transform fit
    strictly inside the training pool -- same rule CLAUDE.md states for
    augmentation, for the same reason.
    """
    n = 0
    total = None
    total_sq = None
...
def _load_data(setname, embeddername, folds_train, name_translation, aug_dirnames,
    ...
    n_passthrough = 0
    if standardize:
        # After the augmented frames are folded in: the statistics describe
        # the pool the model actually trains on.
...
def _train_one(dir_model, modelname, embeddername, setname, name_translation,
    ...
    if data.norm_stats is not None:
        # Per-dim standardization of the input, as the first layer of the
        # model rather than a preprocessing step, so scoring and the shipped
...
def train_set(name, embeddername, setname, name_translation,
    ...
    data = _load_data(setname, embeddername, folds_shipped, name_translation,
                      aug_dirnames, val_fold=None, standardize=standardize)
    result, model = _train_one(
