...
read_annotations <- function(source_set){
  path_annotations <- file.path(dir_annotations, source_set, 'annotations_combined.csv')
...
read_folds <- function(source_set){
  path_folds <- file.path(dir_annotations, source_set, 'folds.csv')
  ...
  path_folds %>% 
    read.csv() %>% 
    mutate(.before=0, set = source_set)
}
