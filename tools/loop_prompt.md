...
- Cleanup is the loop's: once you signal, it stops this session and kills every job you started with `launch_job.sh`, notifiers included. Don't stop jobs or notifiers yourself. If a job is still running when you signal, write its exact relaunch command in the experiment's HANDOFF.md.
